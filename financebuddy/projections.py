...
def _apply_events(connection: sqlite3.Connection, events: list[dict]) -> None:
    for event in events:
        connection.execute(
            """
...
def apply_events(
    db_path: Path, events: list[dict], connection: sqlite3.Connection | None = None
) -> None:
    if connection is not None:
        _apply_events(connection, events)
        return
